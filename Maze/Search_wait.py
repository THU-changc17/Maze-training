...
#每行0 1 2 3分别为每个状态向上、下、左、右移动的行动价值
    def newmaze(self):
        if (self.learning == 1):
            ...
            self.clip_list = []
            self.fire_list = []
            self.Qtable = [[0 for i in range(4)] for i in range(self.ranknum * self.ranknum)]
            # self.Qtable_fire = [[0 for i in range(4)] for i in range(self.ranknum * self.ranknum)]
            ...
            if (self.ranknum < 6 or self.ranknum > 10):
                ...
                self.rewards = [0] * self.ranknum * self.ranknum
                # self.rewards_fire =  [0] * self.ranknum * self.ranknum
                clip_num = 0
                fire_num = 0
                ...
                # for l in self.fire_list:
                #   self.rewards_fire[l] = -10
    ...
    def update_learnui(self):
        ...
        for k in range(self.epoch):
            ...
            while (curr_state != self.state[-1]):
                action_choice = [0,1,2,3]
                ...
                if (random.uniform(0, 1) > self.epsilon):
                    ...
                    #     # else:
                    #     #     action_list = [self.Qtable_fire[curr_state][j] for j in action_choice]
                    #     #     action = action_choice[random.choice(self.get_maxpos(action_list))]
                    # else:
                    action_list = [self.Qtable[curr_state][j] for j in action_choice]
                    action = action_choice[random.choice(self.get_maxpos(action_list))]
                #print(action_choice)
                if (action == 0 and curr_state // self.ranknum != 0):
    ...
    def update_searchui(self):
        ...
        num = 0
        while (curr_state != self.state[-1] and curr_state not in self.clip_list):
            action_choice = [0, 1, 2, 3]
            if (curr_state // self.ranknum == 0):
                action_choice.remove(0)
            if (curr_state // self.ranknum == self.ranknum - 1):
            ...
            if (action == 0 and curr_state // self.ranknum != 0):
                ...
                next_state = curr_state
            if(self.fire.isChecked() and next_state in self.fire_list and num%4 > 1):
                next_state = curr_state
                # print(next_state[i])
            num += 1
            print(str(curr_state) + "+" + str(next_state))
